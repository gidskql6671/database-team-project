...
public class ClassController {
    ...
    public List<Post> getClassPostList(@SessionAttribute(name = "userId", required = false) Long userId,
                                       @PathVariable String lectureCode, @PathVariable String sectionCode) throws SQLException {
        isTakingClass(userId.toString(), lectureCode, sectionCode);
        return classRepository.getPosts(lectureCode, sectionCode);
    }

    // 3. 수강 중인 수업의 게시글 보기
    @GetMapping("/{lectureCode}/{sectionCode}/post/{postId}")
    public Post getClassPost(@SessionAttribute(name = "userId", required = false) Long userId,
    ...
    public List<Comment> getCommentList(@SessionAttribute(name = "userId", required = false) Long userId,
                                        @PathVariable String lectureCode, @PathVariable String sectionCode,
                                        @PathVariable int postId) throws SQLException {
        isTakingClass(userId.toString(), lectureCode, sectionCode);
    ...
    public List<String> getTakingClassStudentList(@SessionAttribute(name = "userId", required = false) Long userId,
                                                  @PathVariable String lectureCode, @PathVariable String sectionCode) throws SQLException {
        isTakingClass(userId.toString(), lectureCode, sectionCode);
    ...
    public int getTakingClassStudentCount(@SessionAttribute(name = "userId", required = false) Long userId,
                                          @PathVariable String lectureCode, @PathVariable String sectionCode) throws SQLException {
        isTakingClass(userId.toString(), lectureCode, sectionCode);
    ...
    @GetMapping("/day")
    public List<ClassInfo> getClassListByDayOfWeek(@SessionAttribute(name = "userId", required = false) Long userId, @RequestParam int dayOfWeek) throws SQLException {
    ...
    public void writeComment(@SessionAttribute(name = "userId", required = false) Long userId,
                             @PathVariable String lectureCode, @PathVariable String sectionCode,
